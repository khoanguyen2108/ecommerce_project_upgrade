...
export function AdminPaymentsPage({ initialQuery }: { initialQuery: AdminPaymentQuery }) {
  ...
  return (
    <div className="admin-resource admin-resource--wide">
      ...
      <div className="admin-table-wrap admin-table-wrap--commerce"><table className="admin-table admin-table--commerce"><thead><tr><th>Payment</th><th>Provider</th><th>Status</th><th>Reconciliation</th><th>Amount</th><th>Provider order code</th><th>Order</th><th>Order status</th><th>Customer</th><th>Failure reason</th><th>Created</th><th>Paid</th><th>Cancelled</th><th>Action</th></tr></thead><tbody>
        {isLoading ? <AdminTableSkeleton columns={14} rows={6} /> : null}
        {!isLoading && !error && payments.length === 0 ? <tr><td className="admin-table__state" colSpan={14}>No payments match the current filters.</td></tr> : null}
        {!isLoading && !error ? payments.map((payment) => <tr key={payment.id}>
          <td><span className="admin-code">{payment.id}</span></td><td>{payment.provider}</td><td><AdminPaymentStatusBadge status={payment.status} /></td><td>{payment.reconciliationIssues.length > 0 ? <><strong>Payment reconciliation required</strong><small className="admin-table__secondary">{payment.reconciliationIssues[0].type} / {payment.reconciliationIssues[0].status}</small></> : "None"}</td><td>{formatCurrency(payment.amount, payment.currency)}</td><td>{formatOrderCode(payment.providerOrderCode)}</td>
          <td><Link className="admin-code admin-table-link" href={`/admin/orders/${encodeURIComponent(payment.order.id)}`}>{payment.order.id}</Link></td><td><AdminOrderStatusBadge status={payment.order.status} /></td><td>{payment.user ? <><strong>{payment.user.email}</strong><small className="admin-table__secondary">{payment.user.name || "Name not set"}</small></> : <strong>Guest customer</strong>}</td><td>{payment.failureReason || "Not set"}</td><td>{formatDateTime(payment.createdAt)}</td><td>{formatDateTime(payment.paidAt)}</td><td>{formatDateTime(payment.cancelledAt)}</td><td><Link aria-label={`View payment ${payment.id}`} className="icon-button admin-icon-button" href={`/admin/payments/${encodeURIComponent(payment.id)}`} title="View payment"><Eye aria-hidden="true" size={17} /></Link></td>
        </tr>) : null}
      </tbody></table></div>
