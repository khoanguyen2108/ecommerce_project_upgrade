  ...
  return (
    <div className="admin-resource admin-resource--wide">
      ...
      <section className="admin-detail-grid" aria-label="Order and customer summary">
        ...
        <article className="admin-detail-card"><p className="eyebrow">Customer summary</p><dl className="admin-detail-list"><Row label="Type" value={order.customerType === "GUEST" ? "Guest" : "Registered customer"} /><Row label="Email" value={order.customerEmail || "Not set"} /><Row label="Name" value={order.customerName || "Not set"} /><Row label="Phone" value={order.customerPhone || "Not set"} />{order.userId ? <Row label="User ID" value={order.userId} code /> : null}</dl></article>
