  ...
  return (
    <div className="admin-resource admin-resource--wide">
      ...
      <section className="admin-detail-grid" aria-label="Payment diagnostics summary">
        ...
        <article className="admin-detail-card"><p className="eyebrow">Customer summary</p>{payment.user ? <dl className="admin-detail-list"><Row label="Email" value={payment.user.email} /><Row label="Name" value={payment.user.name || "Not set"} /><Row label="Phone" value={payment.user.phone || "Not set"} /><Row label="User ID" value={payment.user.id} code /></dl> : <p className="admin-detail-empty">Guest customer; use the linked order for the shipping email and contact snapshot.</p>}</article>
      </section>

      <section className="admin-detail-section"><div className="admin-detail-section__header"><div><p className="eyebrow">Payment reconciliation required</p><h2>Manual review issues</h2></div><span>{payment.reconciliationIssues.length} records</span></div><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Issue type</th><th>Status</th><th>Reason</th><th>Amount</th><th>Provider order code</th><th>Created</th></tr></thead><tbody>
        {payment.reconciliationIssues.length === 0 ? <tr><td className="admin-table__state" colSpan={6}>No payment reconciliation issue is recorded for this payment.</td></tr> : payment.reconciliationIssues.map((issue) => <tr key={issue.id}><td><strong>{issue.type}</strong></td><td>{issue.status}</td><td>{issue.safeReason}</td><td>{formatCurrency(issue.amount, issue.currency)}</td><td>{formatOrderCode(issue.providerOrderCode)}</td><td>{formatDateTime(issue.createdAt)}</td></tr>)}
