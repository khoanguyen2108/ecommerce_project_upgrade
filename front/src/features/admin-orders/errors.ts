...
const ADMIN_ORDER_ERROR_MESSAGES: Record<string, string> = {
  ...
  ADMIN_ORDER_STATUS_INVALID:
    "This order's current status does not allow that action.",
...
export function getAdminOrderErrorMessage(
  ...
  return ADMIN_ORDER_ERROR_MESSAGES[error.code] || fallback;
}
